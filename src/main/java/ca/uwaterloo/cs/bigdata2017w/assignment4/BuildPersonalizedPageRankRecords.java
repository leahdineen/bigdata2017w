...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
  ...
  private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
    ...
    private static final ArrayList<Integer> sourceIds = new ArrayList<>();

    @Override
    public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNode>.Context context) {
      String[] sourceNodes = context.getConfiguration().getStrings("sources");
      if (sourceNodes.length == 0) {
        throw new RuntimeException("List of source nodes cannot be empty");
      }
      ...
      for (String s : sourceNodes) {
      	sourceIds.add(Integer.parseInt(s));
      }
    ...
    public void map(LongWritable key, Text t, Context context) throws IOException,
      ...
      for (int i=0; i < sourceIds.size(); i++) {
      	if(node.getNodeId() == sourceIds.get(i)) {
      		pageranks[i] = 0.0f;
      	} else {
  ...
  public int run(String[] args) throws Exception {
    ...
    //bring back the other way of command line arguments?
